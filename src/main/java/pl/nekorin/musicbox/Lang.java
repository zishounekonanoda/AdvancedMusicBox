...
public enum Lang {
    ...
    LANGUAGE_SWITCH_TITLE(
            "&bLanguage: &e{lang}",
            "&bLanguage: &e{lang}"),
    LANGUAGE_SWITCH_LORE(
            Arrays.asList(
                    "&7Click to switch language",
                    "&7Current: &f{lang}",
                    "&7Available: EN / JA"
            ),
            Arrays.asList(
    ...
    public static void reload(File folder, String lang) {
        ...
        if (!folder.exists() && !folder.mkdirs()) {
            System.err.println("Failed to create lang folder: " + folder.getAbsolutePath());
        }
        if (!langFile.exists()) {
            try {
                // Try to copy bundled resource if present
                MusicBox.getInstance().saveResource("lang/" + lang + ".yml", false);
            } catch (IllegalArgumentException ignored) {
                // Resource not found, will be generated by fill()
            }
        }
        int index;
        if (lang.equals("ru"))
