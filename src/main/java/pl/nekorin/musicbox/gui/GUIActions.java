...
public class GUIActions {
    ...
    private static BarButton languageButton() {
        return new BarButton() {
            @Override
            public ItemStack getItemStack(PlayerWrapper wrapper) {
                String current = MusicBox.getInstance().getConfigObject().getLang().toUpperCase();
                return ItemUtils.createStack(XMaterial.BOOK, Lang.LANGUAGE_SWITCH_TITLE.toString("{lang}", current), Lang.LANGUAGE_SWITCH_LORE.toList("{lang}", current));
            }

            @Override
            public InventoryAction getAction(PlayerWrapper wrapper, SongContainerGUI.SongGUIData<Void> data) {
                return new ClickAction(() -> {
                    String current = MusicBox.getInstance().getConfigObject().getLang();
                    String next = current.equalsIgnoreCase("ja") ? "en" : "ja";
                    MusicBox.getInstance().changeLanguage(next);
                    data.refreshInventory();
                });
            }
        };
